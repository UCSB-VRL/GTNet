...
def get_pool_loc(ims, image_id, flag_, size=(7, 7), spatial_scale=1, batch_size=1):
    ...
    for batch in range(batch_size):
        ...
        # a = copy.deepcopy(
        #     parent.train_test.func_globals['all_utils'][(this_image, flag)])
        a=copy.deepcopy(parent.train_test.__globals__['all_utils'][(this_image,flag)])
        roi_pers, roi_objs, objects_embed = a['person_bbx'], a['objects_bbx'], a['objects_embed']
        # union_box=helpers_pre.get_attention_maps(this_image,flag)

        union_box = a['union_box']
        union_box_out.append(torch.tensor(
            union_box[:, 0:2, :, :]).cuda().float())
        objects_embed_out.append(torch.tensor(objects_embed).cuda().float())
        stats.append([len(roi_pers), len(roi_objs), a['selection_index']])

        W, H, C = ims[batch].size()[1], ims[batch].size()[
                                  2], ims[batch].size()[0]
        ...
        for index, roi_val in enumerate(roi_pers):
            x1, y1, x2, y2 = int(roi_val[0]), int(
                roi_val[1]), int(roi_val[2]), int(roi_val[3])
            sp = [x1, y1, x2, y2, x2-x1, y2-y1]
            sp = [x1, y1, x2, y2, x2-x1, y2-y1]
            sp = [x1, y1, x2, y2, x2-x1, y2-y1]
            im = image_this_batch.narrow(0, 0, image_this_batch.size()[
                                         0])[..., y1:(y2+1), x1:(x2+1)]
            pooled = max_pool(im)
        ...
        for index, roi_val in enumerate(roi_objs):
            ...
            spatial_locs.append(sp)
    return torch.stack(pers_out), torch.stack(objs_out), spatial_locs, torch.cat(union_box_out), torch.cat(objects_embed_out), stats
...
def pairing(pers,objs,context,spatial_locs,pairs_info,stats):    
    ...
    for batch in range(len(pairs_info)):
        ...
        this_batch_selection=torch.tensor(stats[batch][2]).long()
        ...
        tmp=torch.cat([pers_objs_batch,batch_context.repeat(pers_objs_batch.size()[0],1)],1)
        pairs_only.append(pers_objs_batch[this_batch_selection])
        pairs_out.append(tmp[this_batch_selection])
        ...
        del this_batch_selection
  
    return torch.cat(pairs_out),torch.cat(pairs_only)
